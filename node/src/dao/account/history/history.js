...
dao_account_history.select_one = async (history_idx) => {
    let sql = "SELECT h.idx, (SELECT member_idx FROM account WHERE idx = h.account_idx) member_idx, " +
...
dao_account_history.select_daily_situation = async (account_idx, start, end) => {
    let sql = "SELECT SUM(IF(type=0 , amount, 0)) outcome, SUM(IF(type=1 , amount, 0)) income, FROM_UNIXTIME(created_at, '%Y%m%d') day FROM account_history " +
        "WHERE account_idx = ? " +
...
dao_account_history.select_month_situation = async (account_idx, year_month) => {
    let sql = "SELECT SUM(IF(type=0, amount, 0)) outcome, SUM(IF(type=1, amount, 0)) income " +
        "FROM account_history " +
        "WHERE account_idx = ? " +
...
dao_account_history.select_category_info = async (account_idx, type, year, month) => {
    let sql = "SELECT c.name, SUM(h.amount) amount, CONCAT('#',c.color) color, c.idx category_idx FROM account_history h, account_history_category c " +
